...
public class Filter5Service {
    ...
    public String loadVideoFromPPTX(MultipartFile file){
        ...
        Path inPath = Paths.get(applicationProperties.getFilter5VideoInDir())
                .toAbsolutePath().normalize();
        inPath = inPath.resolve(fileName);
        log.info(inPath.toString());
        fileStorageService.storeFile(inPath,file);
        commandExecuter.processCommand(applicationProperties.getFilter5VideoScriptfile()+" "+
                applicationProperties.getFilter5VideoInDir()+fileName+" "+
                applicationProperties.getFilter5VideoOutDir()+" "+
...
//        System.out.println (applicationProperties.getPptxVideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"));
        Path outPath = Paths.get(applicationProperties.getFilter5VideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"))
                .toAbsolutePath().normalize();
