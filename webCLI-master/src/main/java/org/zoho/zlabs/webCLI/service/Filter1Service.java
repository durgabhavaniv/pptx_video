...
public class Filter1Service {
    ...
    public String loadVideoFromPPTX(MultipartFile file){
        ...
        Path inPath = Paths.get(applicationProperties.getFilter1VideoInDir())
                .toAbsolutePath().normalize();
        inPath = inPath.resolve(fileName);
        log.info(inPath.toString());
        fileStorageService.storeFile(inPath,file);
        commandExecuter.processCommand(applicationProperties.getFilter1VideoScriptfile()+" "+
                applicationProperties.getFilter1VideoInDir()+fileName+" "+
                applicationProperties.getFilter1VideoOutDir()+" "+
...
//        System.out.println (applicationProperties.getPptxVideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"));
        Path outPath = Paths.get(applicationProperties.getFilter1VideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"))
                .toAbsolutePath().normalize();
