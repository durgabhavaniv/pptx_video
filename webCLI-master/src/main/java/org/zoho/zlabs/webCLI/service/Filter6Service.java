...
public class Filter6Service {
    ...
    public String loadVideoFromPPTX(MultipartFile file){
        ...
        Path inPath = Paths.get(applicationProperties.getFilter6VideoInDir())
                .toAbsolutePath().normalize();
        inPath = inPath.resolve(fileName);
        log.info(inPath.toString());
        fileStorageService.storeFile(inPath,file);
        commandExecuter.processCommand(applicationProperties.getFilter6VideoScriptfile()+" "+
                applicationProperties.getFilter6VideoInDir()+fileName+" "+
                applicationProperties.getFilter6VideoOutDir()+" "+
...
//        System.out.println (applicationProperties.getPptxVideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"));
        Path outPath = Paths.get(applicationProperties.getFilter6VideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"))
                .toAbsolutePath().normalize();
