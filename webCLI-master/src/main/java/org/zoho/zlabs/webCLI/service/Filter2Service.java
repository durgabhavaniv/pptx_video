...
public class Filter2Service {
    ...
    public String loadVideoFromPPTX(MultipartFile file){
        ...
        Path inPath = Paths.get(applicationProperties.getFilter2VideoInDir())
                .toAbsolutePath().normalize();
        inPath = inPath.resolve(fileName);
        log.info(inPath.toString());
        fileStorageService.storeFile(inPath,file);
        commandExecuter.processCommand(applicationProperties.getFilter2VideoScriptfile()+" "+
                applicationProperties.getFilter2VideoInDir()+fileName+" "+
                applicationProperties.getFilter2VideoOutDir()+" "+
...
//        System.out.println (applicationProperties.getPptxVideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"));
        Path outPath = Paths.get(applicationProperties.getFilter2VideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"))
                .toAbsolutePath().normalize();
