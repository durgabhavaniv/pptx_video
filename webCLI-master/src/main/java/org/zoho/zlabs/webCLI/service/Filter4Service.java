...
public class Filter4Service {
    ...
    public String loadVideoFromPPTX(MultipartFile file){
        ...
        Path inPath = Paths.get(applicationProperties.getFilter4VideoInDir())
                .toAbsolutePath().normalize();
        inPath = inPath.resolve(fileName);
        log.info(inPath.toString());
        fileStorageService.storeFile(inPath,file);
        commandExecuter.processCommand(applicationProperties.getFilter4VideoScriptfile()+" "+
                applicationProperties.getFilter4VideoInDir()+fileName+" "+
                applicationProperties.getFilter4VideoOutDir()+" "+
...
//        System.out.println (applicationProperties.getPptxVideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"));
        Path outPath = Paths.get(applicationProperties.getFilter4VideoOutDir()+fileName.replaceFirst("[.][^.]+$", "/"))
                .toAbsolutePath().normalize();
